...
var filestrs = [];//已经上传的文件 //add by sdy
(function($,undefined){
	$.fn.zyUpload = function(options,param){
		...
		return this.each(function(){
			...
			var defaults = {
					...
					url: "../../tools/file_upload.ashx?action=zyupload",  	// 上传文件的路径
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
				    ...
				    html += '							<img class="upload_image" src="../../scripts/zyupload/control/images/add_img.png" style="width:expression(this.width > ' + imgWidth + ' ? ' + imgWidth + 'px : this.width)" />';
				    html += '						</div>';
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				// 处理不同类型文件代表的图标
				var fileImgSrc = "../../scripts/zyupload/control/images/fileType/";
				...
				var params = {
					...
					onSuccess: function (file, response) {
					    if (response != "") {
					        ...
					        filestrs.push(response);//add by sdy

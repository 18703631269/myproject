<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="type_index.aspx.cs" Inherits="Web.admin.plugins.type_index" %>
...
<body class="mainbody">
    <form id="form1" runat="server">
        ...
        <div class="location">
            ...
            <i class="arrow"></i>
            <span><asp:Literal ID="litF" runat="server"></asp:Literal></span>
            <i class="arrow"></i>
            <span><asp:Literal ID="litT" runat="server"></asp:Literal></span>
        </div>
        <!--/导航栏-->
                ...
                <HeaderTemplate>
                    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="ltable">
                        <tr>
                            ...
                            <th align="left" id="scity" runat="server" visible="false">所属城市</th>
                ...
                <ItemTemplate>
                    <tr>
                        <td align="center">
                            ...
                            <asp:HiddenField ID="hidId" Value='<%#Eval("tid")%>' runat="server" />
                        </td>
                        <td><a href="type_edit.aspx?action=<%#DTEnums.ActionEnum.Edit %>&qx=type_index.aspx&id=<%#Eval("tid")%>&type=<%=haction.Value %>"><%#Eval("tname") %></a></td>
                        <td><%# Convert.ToInt32(Eval("is_lock")) == 0 ? "(未审核)" : "[已审核]"%></td>
                        <td id="tdcity" runat="server" visible="false"><%#Eval("ctype")%></td>
                        <td><%#string.Format("{0:g}",Eval("add_time"))%></td>
                        <td><asp:TextBox ID="txtSortId" runat="server" Text='<%#Eval("sort_id")%>' CssClass="sort" onkeydown="return checkNumber(event);" /></td>
                        <td align="center"><a href="type_edit.aspx?action=<%#DTEnums.ActionEnum.Edit %>&qx=type_index.aspx&id=<%#Eval("tid")%>&type=<%=haction.Value %>">修改</a></td>
                    </tr>
                </ItemTemplate>
                <FooterTemplate>

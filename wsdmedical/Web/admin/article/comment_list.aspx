...
<div class="table-container">
  ...
  <ItemTemplate>
    <tr>
      <td class="comment">
        <div class="title">
          <span class="note"><i><%#Eval("user_name")%></i><i><%#Eval("add_time")%></i><i class="reply"><a href="comment_edit.aspx?qx=comment_list.aspx&id=<%#Eval("id")%>">回复</a></i></span>
          <b><asp:CheckBox ID="chkId" CssClass="checkall" runat="server" /><asp:HiddenField ID="hidId" Value='<%#Eval("id")%>' runat="server" /></b>
          <%#new BLL.articleBll().GetTitle(Convert.ToInt32(Eval("article_id")))%>
        </div>
        <div class="ask">
          <%#Convert.ToInt32(Eval("is_lock")) == 0 ? "<b class=\"audit\" title=\"待审核\"></b>" : ""%><%#Eval("content")%>
          <%#Convert.ToInt32(Eval("is_reply")) == 0 ? "<div class=\"answer\">" +
